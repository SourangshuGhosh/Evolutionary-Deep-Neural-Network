function ind = grow_tree(depth, index)
%grow_tree modified
global F T
global max_depth max_arity
global choose_term

d = ari2dec(index); r = rand;
if depth >= max_depth || (r < choose_term && d > 1)
    i = ceil(rand*length(T.set));
    if strcmp(T.set{i}, 'ERC') || strcmp(T.set{i}, 'erc')
        ind.tree = {num2str(rand)};
    else
        ind.tree = T.set(i);
    end
    ind.tree_index = d;
    ...
    temp = 0;
    for i = 1:f
        child(i) = grow_tree(depth+1, [index dec2ari(temp)]);
