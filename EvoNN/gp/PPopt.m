...
Setslog = {};
Setslog(1) = {'asp_rat2.mat'};
Setslog(2) = {'grainsize2.mat'};
...
F_bad = 1;                %fitness assigned to preys performing badly
%2D-lattice
no_x = 50;                  %lattice size (no of rows)
no_y = 50;                  %lattice size (no of cols)
KillInterval = 5;          %Interval at which bad preys are eliminated
maxrank = 5;               %maxrank retained at KillInterval
...
for generation = 1:no_generations  
    ...
    [F1 F2] = contra( obj(1)*F1, obj(2)*F2);
    F1 = obj(1)*F1;
    F2 = obj(2)*F2;
